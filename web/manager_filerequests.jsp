...
<%
        ...
        <script>alert('File Key Sent');</script>
        <%}
...
<center><h1>Anonymous and Traceable Group Data Sharing in Cloud Computing</h1></center>
	...
	<section id="slider_wrapper" class="under_header slider-wrapper theme-default container">
        ...
        <p><center><font size="5" color="black">Member File Key Requests </font></center></p><br/>
    <style>
        th{
            color: #50aac3;
        }
...
try{ 
        ...
        PreparedStatement pst=con.prepareStatement("select * from memberfile");
...
<%
	...
	while(rs.next()){
            %><tr>
               ...
               <td><a href="manager_filerequests1.jsp?id=<%=rs.getInt(1)%>&groupname=<%=rs.getString(3)%>&email=<%=rs.getString(2)%>"><font color="#0d637d" size="3">Response</font></a> </td>
